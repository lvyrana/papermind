...
阅读器现在会在单页 canvas 渲染失败时自动用更低像素比例重试，并对异常大页面限制渲染尺度，避免把少数
超大页面直接交给浏览器创建高成本画布。若首页仍无法渲染，页面会显示错误类型，并提供「用浏览器打开 PDF」
的原生阅读器出口；页内失败提示也同步增加该出口。这样即使 PaperMind 的 PDF.js 层失败，试用者仍能先读
原文，开发侧也能从截图和 Sentry 同时获得错误阶段、页码和异常名。

新增 PDF 渲染尺度与 canvas 降级候选测试。该修复不改变 OCR 与文字层逻辑，也不承诺修复所有结构损坏的 PDF；
它只保证失败时优先降级尝试，并给用户一个不离开论文流程的继续阅读路径。
