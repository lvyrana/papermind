...
- 划词当下保留浏览器原生选中文字手感，但隐藏 pdf.js / 浏览器原生 `::selection` 蓝底；鼠标松开后新增 `pdfSelectionLayer`，用选中文本重新走 quote 高亮定位逻辑绘制最终蓝条
- 跨行蓝条不再直接采用 `Range.getClientRects()` 的行盒宽度；优先用选中文本在当前页重定位（横向 DOM 文本、纵向 PDF 字形），仅在定位失败时退回词级矩形裁剪，避免跨行拖选向右侧空白铺满
