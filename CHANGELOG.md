...
- 中文 PDF 的活动选区不再直接显示浏览器对透明文字 `span` 的重复着色，改为按行去重、合并并裁掉相邻行重叠的单层高亮，消除竖向深色块和约 1px 的行间叠色
- 选中文字会清理 PDF 提取产生的软连字符、零宽字符、中文标点空格和中文断行空格，划词后的追问、精读与存卡文本更连贯
- 四个划词动作由分散的大胶囊收成紧凑工具条；根据完整可见选区优先放在下方或上方，页底自动翻转，并钳制在视口内
...
- 中文 PDF 实测多行拖选：7 行对应 7 个高亮矩形，相邻矩形无重叠；工具条与选区保持约 10px 间距
- 实测页底选区：工具条自动放到选区上方且不相交
- `npm run build`、本次改动文件的 ESLint 与 `git diff --check` 通过
