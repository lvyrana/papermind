...
- 将论文详情页标题拆成两套字体规则：
  - 英文标题：更柔和的 serif 字体栈，避免显得过黑过硬
  - 中文标题：宋体优先的稳重字体栈，提升中文标题的稳定感
- 英文与中文标题共用统一的蓝色系标题色，但不再共用同一个字体 class
- 收藏详情页与阅读详情页同步使用同一套标题样式，避免不同页面出现不一致

### 标题翻译反馈优化

- 详情页标题“中文/原文”按钮增加失败提示
- 当本地后端未启动、翻译接口失败或模型异常时，不再静默无反应
