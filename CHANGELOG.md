...
试用者在 Windows 电脑上仍遇到首页提示「第 1 页没有成功显示」。这说明 v0.16.7 的窗口化渲染已经把问题
从整页空白推进到了页级错误，但用户仍只能点「重新加载 PDF」；如果失败来自浏览器 canvas/GPU 限制或
该页 PDF 结构异常，反复重载不会改变结果。

随后 Sentry 详情页给出真正异常名：`TypeError: this[#t].getOrInsertComputed is not a function`。该方法来自
新版本 PDF.js 使用的 `Map.prototype.getOrInsertComputed`，部分试用者的 Windows 浏览器尚不支持；渲染重试
不能修复这种运行时兼容错误，因为每次调用同一 PDF.js 代码都会重复失败。

阅读器现在改用 `pdfjs-dist` 的 legacy 主包、worker 与文字层 CSS，该构建会带上所需 polyfill；同时保留单页
canvas 渲染失败时的低像素比例重试，并对异常大页面限制渲染尺度，避免把少数超大页面直接交给浏览器创建
高成本画布。若首页仍无法渲染，页面会显示错误类型，并提供「用浏览器打开 PDF」的原生阅读器出口；页内
失败提示也同步增加该出口。这样即使 PaperMind 的 PDF.js 层失败，试用者仍能先读原文，开发侧也能从截图和
Sentry 同时获得错误阶段、页码和异常名。

新增 PDF.js legacy 导入护栏、PDF 渲染尺度与 canvas 降级候选测试。该修复不改变 OCR 与文字层逻辑，也不
承诺修复所有结构损坏的 PDF；它只保证旧浏览器兼容路径优先、失败时优先降级尝试，并给用户一个不离开论文
流程的继续阅读路径。
...
小范围试用出现三种不同反馈：短论文正常，部分论文显示空白，另有论文提示无法渲染。生产日志确认 PDF
上传与读取接口均为 200，不是文件没有传上去；失败样本中有一份 166 页 PDF，另有一份文件被系统识别为
0 页。阅读器此前会在 140% 缩放下把整篇每一页都转成 Retina canvas 并常驻内存，166 页理论上会占用
数 GB 图像内存，Windows 微信内置浏览器和内存较小的设备会先中断下载或让 canvas 静默失败。单页异常又
只写 `console.warn`，既没有用户可见出口，也没有进入 Sentry，因此同一类失败会分别表现为空白和报错。

阅读器改为先建立轻量页面骨架，只渲染当前页前后各 2 页；离开窗口的 canvas 主动清零释放，跳页、回到
原文和自测 OCR 需要某页时再按需渲染。30 页以内论文仍在后台提取每页文字，苏格拉底自测的全文上下文
不因窗口化退化。文档加载、页面准备和页面渲染异常现在按阶段与页码进入 Sentry，页面本身显示失败原因
和重新加载出口，不再留下无说明的空白。

新增 166 页与 0 页边界回归测试，验证首页、篇中和末页的 canvas 窗口始终只有 3–5 页；前端 27 项测试、生产
构建和 lint 均通过。真实 43 页 PDF 本地验收保留 43 个页面骨架：首页 canvas 为 1–3 页，滚到第 21 页
只保留 19–23 页，末页只保留 41–43 页，三处均无渲染错误。构建仍保留既有的 pdf.js 4.x 兼容分支
warning 与大 chunk warning。

### Edge 自动翻译后进入论文页偶发整页空白

Sentry 捕获到一条生产环境未处理异常：Windows Edge 用户从首页进入 `/paper/20` 时，React 执行
`insertBefore` 发现目标节点已不在原父节点。Session Replay 同时显示英文 PDF 文字层已被浏览器自动翻译为
中文，说明 React 管理的 DOM 在路由切换前被外部翻译机制改写；异常发生在导航瞬间，不是 PDF 下载失败，
该次 PDF 随后仍显示到第 2/16 页。

应用根节点现在同时使用标准 `translate="no"` 与 `notranslate` 标记，阻止浏览器翻译器直接改写 React DOM；
顶层增加 Sentry ErrorBoundary，极少数未处理前端异常会显示「重新加载 / 返回首页」出口，不再只留下空白页。
新增静态回归护栏，防止后续重构移除这两道保护。

排查还发现 Replay 原配置为 `maskAllText: false`、`blockAllMedia: false`，会把论文正文、选区引用和卡片内容
完整传入错误回放。现改为默认遮罩全部文字并阻止媒体录制；仍保留异常类型、堆栈、路由、浏览器环境与网络
请求状态，足以定位工程故障，但不再用用户阅读内容换取调试便利。

## v0.16.6 - 2026-08-03

### 设置页文案去掉内部说法，每日额度上调

设置页把「试用主持人」「试用者」「专属链接」这类只有内部才懂的说法换成正式表达：AI 服务卡片删去
「配置由试用主持人统一管理」一条（卡片副标题已说明由系统统一提供），「复制我的专属链接」改为
「复制同步链接」，设备编号说明改为解释它的实际作用与分享风险。

退出设备身份原本写着「不会删除已保存的数据」——既然产品没有提供删除入口，这句话对用户没有意义。
改为说明退出后的真实后果：本浏览器会获得新的设备编号，需凭同步链接才能重新访问原有数据。

每人每日额度从 20 次对话 / 30 次翻译上调到 60 / 120。这份 chat 额度实际被四个入口共用——对话、
精读带读、选区与整页 OCR、卡片草稿与自测，一篇论文走完自测就要按页 OCR，旧的 20 次读一两篇即耗尽。
全局熔断保持 500 不变：chat 走 `qwen3.7-max`，单价比 flash 高一个量级，这个值直接决定每日成本上限，
应按预算显式调整而不是顺带提高。

### 导出菜单去掉说明文字，自定义模型移除 OpenRouter 并支持一键申请

汇报板导出下拉原本每项都带一行说明（「.pptx · 双击即开，可直接讲」），格式名本身已经足够清楚，
两行文字反而让菜单变重；现在只保留 PowerPoint 与 Markdown 两个名字，菜单宽度同步收窄。

自定义 AI 模型移除 OpenRouter 预设，默认服务商改为 DeepSeek，后端 `preset` 默认值同步更新。
其余服务商的说明后面新增「获取 API Key」直达链接，指向各家创建 Key 的控制台页面（新标签打开），
不必再自己去翻文档找入口。已保存的 OpenRouter 配置仍可继续使用，只是不再出现在预设列表里。

## v0.16.5 - 2026-08-02

### 图表「送到汇报」卡在"正在投递…"且无法重试

精读台框选图表后选择板块，投递会长时间停在"正在投递…"，最终既没有条目进板，也没有任何提示。

排查确认后端与上传链路正常：线上 `POST /api/board/{id}/figures` 端到端实测 0.78 秒成功，4.3MB 图片
上传耗时 3.3 秒；线上前端构建与仓库源码一致。真正的问题在前端这条投递路径缺少容错——图表上传使用
没有超时的裸 `fetch`（同页 OCR 已配 `timeoutMs` 与重试，唯独这里遗漏），网络一旦卡住就会无限转圈；
而 `boardSending` 又会吞掉后续所有点击，用户既看不到原因也无法重试。后端返回 `{ok:false}` 时同样
只是悄悄停转、弹层保持原样，看起来就是"点了没反应"。

投递现在有 30 秒超时，超时、网络中断和后端拒绝都会在选单内显示具体原因并解除按钮禁用，可以直接
重新点选板块重试；文字条目投递也补上了同样的超时。

## v0.16.4 - 2026-08-01

### Zotero 插件在取消访问密码后仍强制索要账号和密码

线上试用关闭 HTTP Basic Auth 后，PaperMind 网站已可直接访问，但 Connector `0.2.x` 仍把
“线上地址必须同时保存预览账号和密码”写成配置成功的硬条件。结果是网站能打开，插件却持续弹出
已经失效的凭据输入框，旧用户也无法直接沿用原来的专属链接。

Connector 升级到 `0.3.0`：请求只携带匿名设备 ID，不再生成 `Authorization` 头；启动时自动把
`0.2.x` 配置迁移到新版，保留服务地址和设备 ID，并清除 Zotero 首选项中残留的旧密码。
...
随后按作者确认将插件源码授权为 `AGPL-3.0-only`，补齐隐私、安全与支持政策，并发布正式
[`v0.3.0`](https://github.com/lvyrana/papermind-zotero/releases/tag/v0.3.0) XPI。主仓库部署镜像同步许可证声明和
同一 Release 资产，旧安装仍可沿原更新地址升级；正式身份、双语界面和多版本测试继续作为市场提交门槛。

### 前端路由依赖落入多项已披露风险区间

发布前生产依赖审计发现 `react-router-dom 7.13.2` 命中反序列化、重定向和拒绝服务等多项安全公告。
升级到同一主版本的 `7.18.2` 后，已消除当前客户端路由实际使用路径上的相关告警。审计仍会报告一项
仅针对 React Server Components action 的公告；PaperMind 使用纯客户端 `BrowserRouter`，未启用 RSC，
因此记录为不适用的残余告警，而不为清零审计结果进行不必要的大版本改造。

---

## v0.16.3 - 2026-07-31

### 试用期关闭 HTTP Basic Auth

试用者反馈手机打开一直 401。查证为 nginx 层的 `auth_basic`（私有预览密码）——
电脑上早已输过并被浏览器记住，故只在新设备暴露；且手机上输入密码后仍反复弹框。

试用只有几天窗口，多一道门槛就多掉几个人，故先关闭域名与备用入口的密码。
线上 HTTPS 配置由 Certbot 保存在 `/etc/nginx/sites-available/`，并非仓库模板；仅运行
`update.sh` 不会同步这项修改。新增 `deploy/toggle-preview-auth.sh`，在保留证书配置的前提下
同时切换两份真实站点配置，修改前自动备份，且只在 `nginx -t` 通过后 reload。

数据仍按匿名设备 ID 隔离，AI 调用另有用户级和全局每日限额；但设备 ID 不是正式登录鉴权，
因此此次只作为短期公开试用。新增 `robots.txt` 阻止搜索引擎主动收录，试用结束后用同一脚本恢复密码。

同步更新试用指南：去掉「共享访问密码」，并明确建议用电脑浏览器
（划词与 PDF 阅读在手机上不便操作）。
...
带读面板最高占 46vh、自由笔记再占 38%，卡片流被挤到只剩缝隙，只能一个个手动收起。
改为**手风琴**：展开带读自动收起笔记，反之亦然；带读高度上限 46vh → 32vh。

### 精读结果面板无法关闭

结果面板没有任何关闭入口，读完一段再读一段就越堆越多，既无法继续也无法关掉。
标题栏新增收起按钮。

### 「问 papermind」与「精读这段」重复

两者都是针对选中段落问 AI，区别仅在于前者要用户自己打字。合并为「精读这段」；
原有能力不丢——精读时同时把该段挂进对话待引用，想自己追问直接在对话里打字即可。

### 对话总结成笔记后无法进入汇报板

总结只写进笔记，没有任何送汇报入口。总结成功后新增「把这份总结送到汇报板」。

### “总结送到汇报板”按钮可见但点击后没有内容

后端总结接口返回字段为 `note`，前端首版却只读取 `summary/content`，导致保存成功后
用于汇报板的正文仍是空字符串。前端改为优先读取 `data.note`，并在清空对话或继续提问时
同步清除旧总结，避免把过期内容送入汇报板；新增护栏测试锁定接口字段。

---

## v0.16.1 - 2026-07-31

### 导出 PPT 不分页，长板块文字溢出到页面外

原实现「一个板块固定一页」，内容多少都塞进同一个文本框。PPT 文本框不会自动分页，
用户实测「方法」页文字直接跑出画面。

改为按容量估算分页：正文区约容纳 18 行、每行约 34 个中文字（15pt / 11.5 英寸宽），
超出即新开一页，标题加「（续）」。图表条目改为独占一页，不再和正文抢空间。
实测同一篇论文：原 8 页（其中 1 页严重溢出）→ 10 页，无溢出、无空白页。

### PPT 分页后单页仍溢出（换行未计入行数）

首版分页只按「总字数 ÷ 每行字数」估算，但卡片正文常含小标题与「1. 2. 3.」编号，
一段里有多处换行时真实行数远高于估算值，于是分页生效了、单页却依然溢出。
改为**逐段折行**累加，并把每行字数从 34 收紧到 46、单页上限降到 15 行（留约 25% 余量）。
部署前复核又发现“单个超长条目”仍会被整块放进一页，因此继续把条目内部的视觉行按剩余容量切片，
确保一张含多段编号的长卡片也能跨续页，而不只是多个卡片之间分页；新增单条长卡片回归测试。

### 汇报板里的导出菜单被浏览器工具栏挡住

导出菜单固定向上弹出（`bottom-full`）。在右栏底部合适，但汇报板抽屉的按钮在页面顶部，
向上弹会跑到浏览器工具栏后面，完全看不见。改为按位置选择方向，抽屉内向下弹。

### 对话记录无法删除

聊过的内容有时只是当下随手一问，但界面上没有任何删除入口，等于强迫留痕。
对话抽屉标题栏新增清空按钮（有对话时才显示），同时清理本地缓存与后端记录。
新增 `DELETE /api/chat/{paper_rowid}`（校验归属）与 `clear_chat_history()`。
...
根因是 v0.12.5 隐藏画像表单时**只砍了输入端没砍消费端**：`focus_areas` / `background`
等字段仍被塞进每次对话的 system prompt，`memory_core` 又是从这份表单生成的——
于是 AI 长期引用一份用户早已不再维护、且完全看不见的旧描述。

**记忆改为从精读行为学**
- `ensure_memory_core` 不再读画像表单，改为读 `get_deep_reading_signals()`：
  精读过的论文、自己沉淀的卡片、追问过的问题
- 去掉「必须先填画像才有记忆」的前置门槛
- prompt 写死「只依据行为证据，不要编造没出现过的研究方向或身份」

**清干净幽灵表单**
`update_memory_recent` / `merge_recent_to_core` / `maybe_auto_refresh_memory_core`
三处 prompt 里都还标着「用户明确画像（优先级最高）」并注入那些隐藏字段——
只清对话那一处不够，记忆一重算就会长回来。三处一并清除。

**所见即所得**
`_build_understanding_profile_text` 现在只返回记忆面板里看得见的内容。
面板里改了什么，AI 拿到的就变什么；删空了它就什么背景都不知道。

**「你说过的」跨对话保留**（新增 `memory_stated`）
- 从对话里只挑**明确讲过的自我陈述**，提问不算（问「PSM 怎么用」≠ 在做 PSM 研究）
- 与阅读画像分开存：画像可对着论文和卡片查证，这里可对着某次对话查证；
  混在一段里就又变成无从核实的话
- 沿用原记忆系统最值钱的设计：**增量更新**——已有条目喂回去只做新增合并，
  不推倒重写（这是原来「感觉记得住」的来源）
- 逐条可删；喂给 AI 时标注「可信度高于以上推断」

**使用规则**（治「什么都往记忆上扯」）
只在与当前论文确实相关时才引用；不相关就闭嘴。聊到课题启发、以前做过什么、
对什么感兴趣时，才是该主动调用背景的时候。

**面板**：书架画像卡点开展开 → 阅读画像（可改写/重新生成/清空）+ 你说过的（逐条删）
**首页**：不动，仍是最近阅读那一行

新增接口：`GET/PATCH/DELETE /api/memory`、`POST /api/memory/rebuild-core`、
`POST /api/memory/refresh-stated`、`POST /api/memory/stated/delete`。
清空会连同早年表单残留字段一起清，避免借尸还魂。
...
- 中文 PDF 的活动选区不再直接显示浏览器对透明文字 `span` 的重复着色，改为按纵向重叠率识别同行片段、逐行去重合并并裁掉相邻行重叠的单层高亮；同时移除覆盖层内描边，减少竖向深色块和行间接缝
- 选中文字会清理 PDF 提取产生的软连字符、零宽字符、中文标点空格和中文断行空格，划词后的追问、精读与存卡文本更连贯
- 四个划词动作由分散的大胶囊收成紧凑工具条；定位改为上方优先、左右其次、下方最后，并逐一检查工具条与每一行选区的间距，避免长段选区被工具条压住
- Founder PDFGenerator 等缺少 `ToUnicode` 映射的中文 PDF 会先检测乱码，仅裁取实际选中的文字行交给千问 OCR；识别完成后才开放追问、精读、存卡和送到汇报，识别失败时明确阻断，不再保存乱码
- 中文论文中的正常英文标题、摘要和表头不再因拉丁字符占比高而被误判为乱码；OCR 状态不再替换整组划词操作，识别期间四项操作保持可见但禁用，失败后可原地重试
- OCR 模型路由显式排除不支持图片输入的 `qwen3.7-max` 别名，同时保留已验证可用的日期快照与视觉模型，避免无效的 400 请求拖慢识别
- 卡片 AI 起草在存在划选原文时不再接收论文摘要，并通过提示词和服务端乱码校验双重限制证据边界，禁止补入选区外的指标、数字、步骤或结论
- OCR 选区保持激活时，`Command/Ctrl + C` 会把剪贴板中的底层乱码替换为已识别文字，视觉选中与实际复制结果保持一致
- 苏格拉底自测打开前会检查全文文字质量：30 页以内论文只对乱码页逐页 OCR，并按论文与页码缓存识别文本；自测使用带页码标记的全文出题、判定和逐字锚点校验，页面图片不落盘
- 全文缓存读取增加短超时和 3 次重试，单页 OCR 增加明确超时与失败页码提示；网络抖动不再让自测入口永久停在“正在读取全文”
- 精读结果到达时的闪烁提示改由 CSS animation 驱动，移除 Effect 内同步更新状态，恢复 `PaperRead.jsx` 的 React Hooks Lint
- 用户可见的「组会汇报板」统一缩短为「汇报板」，导出文件名前缀同步调整；“送到汇报”等动作文案保持不变

#### 验证

- 中文 PDF 多行拖选：同行碎片按纵向重叠率归并，相邻行在中点处分界，覆盖层不再叠加内描边
- 选区定位单测覆盖普通选区、长段中文选区、上方空间不足、仅下方可用及超宽工具条，共 5 项
- 以《基于生成式人工智能的护理证据智慧问答模型的开发与可用性评价》第 3 页真实裁图验证：OCR 正确恢复监督微调、70/80 万条数据、5S 重排序和模型交付；起草未混入故意放在摘要中的 BLEU、Likert
- 同文第 2 页整页 OCR 正确恢复“准确性和可溯源性不足”等前言原句，能够为自测答案提供可逐字核验的中文语料
- 中英混排判定、OCR 模型能力共 7 个新增/既有边界用例通过；后端守卫测试 29 项、前端文字与重试测试 10 项通过
- `npm run build`、本次改动文件的 ESLint、定位单测与 `git diff --check` 通过

### Zotero 插件恢复线上连接

- PaperMind Connector 升级到 `0.2.0`，默认连接 `https://papermindapp.com`，不再把正式部署后的用户引回 `127.0.0.1:8000`
- 首次连接改为直接粘贴设置页复制的专属链接，自动提取域名和严格 UUID v4 设备 ID，避免手工拆 uid
- 线上私密预览支持 Basic Auth；连接测试、文献保存和 PDF 上传统一携带认证与设备身份
- 网络、预览认证、设备身份和旧本机地址分别给出可操作的错误提示
- 阅读深链明确带上 `library=1`，避免数字 ID 被误当成推荐缓存论文，确保直接加载刚上传的本地 PDF
- 新增公开但不接触用户数据的插件更新清单与 XPI 下载入口，并补齐打包脚本、插件单元测试和后端包校验测试
...
- 匿名设备 ID 改为严格 UUID v4 校验；缺失或异常身份不再回退到共享的 `anonymous` 数据空间
- 设备 ID 同时保存在 localStorage 与同源 cookie：普通 API 使用 `X-User-ID`，PDF 与图表等媒体请求用 cookie 校验归属
- 补齐跨设备归属检查：RIS/BibTeX 导出、阅读历史、项目归属和笔记更新均不能引用其他设备的数据
- 新设备默认进入独立空间；专属链接仍作为用户主动选择的多端同步凭证，并继续兼容 Zotero 一键精读深链
- 自定义 AI 模型属于主机级配置，仅 `OWNER_UID` 对应的主持人设备可查看和修改
- 删除论文时同步清理苏格拉底自测并解除历史引用；按设备删除脚本补齐 `self_test_sessions` 与 `method_gaps`
- 后端测试由 8 项扩充到 16 项，新增双设备存储、交叉访问、主机配置权限与删除完整性覆盖

---

## v0.14.0 - 2026-07-26

### 苏格拉底自测——让 papermind 考考你（handoff 第 6 项，本期唯一全新功能）

它是**诊断工具，不是老师**：唯一职责是照出盲点，不负责填平盲点（填平交给对话）。

**核心问题（五个，界面统一称「核心问题」）**
研究问题 / 方法—问题匹配 / 结果有多强 / 哪里站不住 / 外推边界。一次只问一个，一根问到底。

**出题依据混合三来源**（只考卡片＝只考你已经会的）
① 已有卡片 ② 划了但没做成卡片的段落 ③ 完全没碰过的汇报板板块——prompt 里显式要求**优先从 ②③ 出题**。

**判定三档，宁可较真不可捧场**
`站住了 / 部分对(缺哪块) / 还不对(问题在哪)`；prompt 铁律禁止「很好的观察」这类捧场开场。
校准过一次：最初「原文没写就判 off」会误杀用户带入的正确方法学常识（如 MCID），
改为**只有与原文冲突或理解错误才判 off**，带入正确外部知识反而算读进去了。

**原文锚点由后端逐字校验**（`_verify_anchor`）
模型给的 `anchor_quote` 必须真的出现在原文里；校验不过就剥掉锚点、并把 solid 降级为 partial。
「没有锚点的反馈一律不可信」的机器执行版——不靠模型自觉。

**三问收口**：由后端 `turn_count` 强制，不由模型决定，避免「一根问到底」变折磨。

**「不确定 · 转到对话」往返**
去：生成预填 prompt + 起手 chips **直接写进对话流**并切到对话，用户不必自己切、自己重述；
回：该问题标记 `已去问`（自测停在这等着，不算失败），提供 `返回作答 ↩`。

**方法学盲区累积**（画像的另一半）
答不上来暴露的概念记入 `method_gaps` 跨论文累加，`GET /api/portrait` 一并返回。

**状态四态，绝不打分**：还没问(空心) / 还模糊(coral) / 已去问(coral半透+ring) / 已厘清(mint)。
分数是伪科学也让人焦虑，用状态图代替。

**产物接回主线**：只有判为「站住了」的回答才给「沉成卡片」入口——避免把错的东西沉进汇报板。

技术细节：新增 `self_test_sessions`、`method_gaps` 两张表；4 个接口
（`GET /api/self-test/{id}`、`/ask`、`/answer`、`/handoff`）；自测不占用每日对话额度，
仅保留 `SELF_TEST_DAILY_LIMIT`（默认 300）作成本熔断。无 PDF 时降级用摘要+卡片+划词作可引用原文。

---

## v0.13.0 - 2026-07-26

### 书架 + 精读台右栏 V1（对齐《精读工作台 结构 Handoff》）

延续 v0.12.5 的产品转向（推荐器 → 精读工作台），本轮做 handoff 第 4、5 项。
第 1、2 项（导航、设置页）已在 v0.12.5 完成；第 6 项苏格拉底自测下一轮单独做。

**书架（原收藏页）**
- 桌面改为单栏 1080：页头「我的书架」+ 统计行（共 N 篇 · 在读 · 读过 · 卡片 · 笔记）
- 新增**画像卡**：深色置顶大卡，一句话总结 + 主题分布 + 卡片四类构成条形图 + 「近 30 天 · N 篇精读」。**完全由行为聚合，无任何输入控件**（画像从「用户填写的偏好」变为「只读副产品」）；不足 3 篇精读时整卡不渲染
- 新增筛选条 `全部 / 在读 / 读过 / 有导出` + 标题搜索；工程行与首页同构，点进真精读台（`/paper/:id?library=1`）
- 后端新增 `GET /api/portrait`（`get_portrait`）：按 category 聚合主题分布、按 card_type 聚合卡片构成、近 30 天精读篇数
- 项目（任务型收藏夹）逻辑保留在 `projectApi`，单栏化后暂无入口，待新 IA 安排位置

**精读台右栏 V1（六块 → 两层）**
- 新结构：顶栏（精读沉淀 · 对话按钮 · ⋯ 溢出菜单）→ 折叠「怎么读这篇」→ **卡片流（flex-1 主体）** → 自测入口 → **钉底汇报板**
- **对话不再常驻**：从占据半个轨改为顶栏按钮唤出的覆盖式抽屉（对话是手段，产物才住在轨里）
- **元操作收进溢出菜单**：收藏、归入项目、自由笔记
- **带读引导收成一行**，点开才展开三态（路线图 / 摘要带读 / 当前页陪读）
- 汇报板底部按钮文案统一为 `导出 PPT`
- 自测入口按 handoff 预留占位（标「即将上线」），功能下一轮实现
- 实现方式为**容器重排**：`CardDrawer` / `BoardRail` / `DeepReadPanel` 新增 `variant="rail"`，内部逻辑（卡片增删改、送到汇报、marp 导出、页码锚点）完全复用未重写
...
- **发现流去留**：「for you」推荐流暂时下线，代码以 `SHOW_LEGACY_FEED` 旗标保留（置 true 即恢复）；W2 精读工作台成为桌面、窄窗口和移动端的统一首页
- **响应式收口（07-26）**：修复窗口小于 `lg` 时误切回旧 `MobileHome` 的问题，不再出现「为你探索」「研究画像待填写」等旧入口；工作台卡片会按宽度自动换列，手机端保持单列且无横向溢出
- **继续精读恢复（07-26）**：本机没有 `last-reading` 缓存时，改由书架中最近的「在读」论文恢复继续精读卡片，避免切换地址或设备后首页看起来像空白新用户
...
- **顶部导航改 IA**（共用 `Navbar`，四页统一）：`收藏` → `书架`；隐藏 `画像`（改为行为自动生成的只读摘要，后续嵌入书架页顶部）；最终为 **首页 / 书架 / 设置**
- **隐藏所有「主动填写偏好」入口**：产品转向精读工作台后画像不再需要用户主动填写，设置页「研究偏好」填写卡（研究方向/方法兴趣/不想看的内容/学科领域/自由描述/检索范围）整块以 `SHOW_PREFERENCES=false` 隐藏（保留代码，置 true 恢复）
- **清推荐流残留**：用量统计三格→两格（去掉「推荐批次」）；配额文案改「每人每天最多 20 次 AI 对话、30 次翻译」，去除推荐字样
- **文案去口语化**：页头副标题「管理 AI 服务、数据与隐私」；反馈卡副标题「问题反馈与功能建议」、成功态「反馈已提交」（去 emoji）、三类占位符改为客观描述
---
...
- 新增 `scripts/backup_local.sh`：本地一键备份数据库（sqlite3 在线备份，后端运行中可执行）+ `--with-files` 打包 PDF 与图表截图；支持 `BACKUP_DIR` 直接写入移动硬盘或受控云盘目录，默认保留 30 天
- 新增 `scripts/delete_user_data.py`：按匿名设备 UUID 预览/删除对应数据库记录、PDF 与图表截图；默认只预览，确认执行前自动备份数据库
- `papermind/.env.example` 补齐代码实际读取但示例缺失的变量：`EMAIL`（PubMed 联系邮箱）、`LLM_TASK_*_MODELS`（六个按任务路由的模型列表）、`CATEGORIZE_MAX_WORKERS`
- 新增 `docs/trial-guide.md`：小范围试用指南；设置页同步修正数据存储、匿名 UUID、专属链接和退出设备身份的真实边界
- 精读带读自动区分中英文：中文模式聚焦研究逻辑、方法、证据、数字和论证，英文模式在同一闭环上增加术语与长句辅助
- 新增 `docs/chinese-reading-validation.md`：用固定流程和致命错误标准验证中文论文核心精读闭环，再进入英文增强阶段
- README：版本对齐 v0.12.5，新增「数据与备份（本地运行）」「小范围试用」章节
- `.gitignore` 补 `backups/`、`*.db-shm`、`*.db-wal`
---
...
- 用户持续反馈拖选时蓝色选区"色条太宽"、从段首下拉两行会选中别处。第一性原理定位：**原生选区高度 = 文字层透明文字的字号**，与我们所有高亮绘制代码无关
- 根因：pdf.js 5.x 把字号公式的变量改名为 `--total-scale-factor`，我们只设了 4.x 旧名 `--scale-factor` → calc 失效 → 全部 span 字号回退浏览器默认 **16px**。画布字形 10px、透明文字 16px：选区带 22px 压上下行；隐形文字盒上下重叠导致鼠标命中测试跳行
- 补设一行 `--total-scale-factor` 后：正文选区带 22px→14px（行距 16px，逐行紧贴不叠行）；命中测试恢复正常
- 教训：此前（含 Codex 历次）对高亮绘制端的修改都在治下游症状——选区体验的病根在文字层本身

#### 已知残留

- 跨栏拖选仍会按 DOM 阅读顺序淹到另一栏（与 Firefox 内置 PDF 阅读器行为一致，原生选区固有限制；Zotero 用自研选区引擎规避）。修复字号后栏间沟槽的盒重叠消除，误触已大幅减少

---

## v0.12.3 - 2026-07-07

### 划词高亮 Zotero 级精确（横向 DOM / 纵向 PDF 字形杂交）+ Retina 截图修正

- 用户反馈高亮仍不如 Zotero 精确。像素级诊断：该期刊 PDF 两端对齐排版，词间距用 TJ 字距位移实现——pdf.js `item.width` 只含字形宽（实测 60 字符行 201/339 user units），按字符比例插值横坐标天然失效；DOM 文字层横向被 pdf.js 拉伸校准（准），但行盒含行距（纵向胖，色带压到下一行字头）
- 修法取两者所长按行配对：**x/宽 ← DOM Range 重定位，y/高 ← PDF 字形坐标**（基线+字高）。实测宽度与文字 ±1px、高度贴住字形不越行、行末过冲从 192px 降到个位数
- **图表截图 Retina 修正**：canvas 背景存储为 CSS×outputScale（视网膜屏 2 倍），裁剪源坐标未乘导致框选错位半屏——已修，且按背景分辨率导出，截图清晰度翻倍。教训记录：dpr=1 的 headless 测试机上 E2E 曾漏过此问题

---

## v0.12.2 - 2026-07-07

### 图表截图入汇报——框选 PDF 上的图/表，直接进组会 PPT

- PDF 工具栏新增「截取图表」：点击进入框选模式（十字光标，Esc 退出），在图/表上拖一个框 → 弹板块选单（带图片预览）→ 选板块入汇报板
- 图片存后端（`data/figures/`，删条目/删收藏级联清理）；汇报板抽屉直接显示图片
- **导出 PPT 时图片进片子**：Marp 导出把图片 base64 内联进 md（单文件自包含），`![h:420]` 限高，marp-cli 实测编译出带图 pptx
- 图片服务端点允许 `?uid=` 查询参数鉴权（`<img>` 标签无法带自定义 header，沿用深链模式），带路径穿越防护与归属校验
- 顺手修掉已知问题：删除收藏现在同步清理对应 PDF 文件与图表文件

#### 验证

- Playwright：框选摘要区域 → 选单出现图片预览 → 落库 source=figure → 抽屉图片真实渲染（naturalWidth=360）→ 导出 md 含内联图（44KB）→ marp-cli 编译出 1.1MB 带图 pptx，全过

#### 待打磨

- 汇报板抽屉里带读长文条目未折叠，占屏过多（下轮 line-clamp + 展开）

---

## v0.12.1 - 2026-07-07

### 划词高亮精确化（Zotero 级）+ 带读视线引导 + 引用可删除

- **高亮从"像素快照"改为"文本重定位"**（第一性原理：高亮锚定的是文本不是像素）：绘制时在当前文字层里重新定位 quote 原文，从真实 DOM 几何推导矩形——任何缩放下都精确贴字；文字层不可用才退回矩形快照。旧数据自动受益
- 矩形按行合并 + 去重复：原来浏览器给的重叠矩形叠色成深浅不一的色块（大段选择"血染"整段），现在 4 行选择就是 4 条干净色带
- 样式减淡：24% 填充 + 描边 → 15% 无描边，行框上下各收 10% 贴文字本体
- 「精读这段」视线引导：动作在 PDF 中间、结果在右栏——现在生成一开始右栏自动滚到精读工作台，结果到达时边框闪烁提示
- 「引用与追问」卡片悬浮删除按钮：删除引用同步擦除 PDF 高亮（原来只能永远积累）

#### 验证

- Playwright：跨 4 行选择 → 恰好 4 条高亮矩形、样式 .15 无描边、缩放 155% 后仍贴住文字、删除后高亮清零、带读触发后工作台滚回视口，全过

---

## v0.12.0 - 2026-07-07

### 组会汇报板（Presentation Board）—— Read → Think → Present 的 Present 层

把组会 PPT 从"读完之后的导出产物"倒转为"读之前就存在的容器"：打开论文即有汇报骨架，边读边把内容投递进对应板块，PPT 完成度即精读完成度。

#### 数据与接口

- 新表 `presentation_boards`（板块结构 JSON + why_reading）与 `board_items`（条目：板块/内容/原文引用/页码/来源/排序）；删除收藏级联清理
- 惰性建板：首次访问自动创建默认 7 板块（研究背景/研究问题/方法/关键结果/批判与局限/对我的启发/讨论问题），板块用稳定英文 key 存储、可增删改名；why_reading 默认取推荐理由
- 6 个接口：`GET/PATCH /api/board/{rowid}`、条目增删改、`GET /api/board/{rowid}/export/marp`

#### 四条投递入口（全部弹板块选单，未收藏先自动收藏）

- 划词浮窗第 4 按钮「送到汇报」（带原文 + 页码）
- 精读工作台带读结果「送到汇报」
- AI 回复「送到汇报」（归卡旁）
- 卡片「入汇报」：类型→板块默认映射（方法卡→方法、发现卡→关键结果、批判卡→批判与局限、迁移卡→对我的启发），选单高亮"推荐"可改投

#### 右栏汇报板区块 + 全览抽屉（新组件 BoardDrawer.jsx）

- 右栏实时显示 x/7 进度，板块 chip 有内容 mint 高亮带数、空灰显——空板块 = 还没读懂的部分
- 全览抽屉：首页信息自动生成（标题/作者/期刊/年份/DOI + 可编辑"为什么读这篇"）、分板块整理条目（编辑/改投/删除/页码回跳）
- 「导出 PPT」下载 Marp Markdown（白底黑字极简），空板块出"待填入"占位页；`marp board.md --pptx` 一步出真 PPT

#### 验证

- 数据层 9 项 + API 层 7 项（TestClient + 临时库）全过
- 独立测试服务（8010、全新库）Playwright 分四步 E2E：右栏/抽屉挂载 → 划词投递 → 真实带读+对话投递 → 卡片入板+导出下载，全过
- 导出的 md 经 marp-cli 实际编译出 869KB pptx
- 顺手修：pub_date "09/2026" 格式年份提取错误（前后端）；Marp bullet 多行续行缩进；划词条目 quote 重复输出
