...
> 版本基线：v0.12.5（2026-07-13，中文核心闭环验证阶段）
...
| 入口 | 路径 | 说明 |
|---|---|---|
| **Zotero 一键精读** | Zotero 里右键文献 →「用 PaperMind 精读」 | 插件自动发送元数据 + 本地 PDF 附件，浏览器直接打开阅读页（`?uid=` 自动认领身份）。首次使用只需在连接设置中粘贴 PaperMind 设置页复制的完整专属链接 |
...
- **划词浮窗**：在 PDF 正文划选 ≥8 个字符的文字，松开鼠标弹出三个按钮：
  ...
  - `精读这段` —— 中文论文做论证拆解，英文论文增加句子与术语拆解（见工作台）
  - `存为卡片` —— 选中文字自动成为卡片的原文锚点（含页码）
- 限制：扫描件没有文字层，划词和按页带读均不可用（OCR 在远期清单）

### 右栏：精读工作台（v0.11.0，核心能力）

三种带读模式，由 `/api/deep-read/guide` 生成。接口会按题名、摘要和当前原文自动区分中英文：中文模式聚焦研究逻辑、方法、证据和数字，英文模式在同一闭环上增加术语与长句辅助；两者都会注入研究者画像并鼓励迁移到用户自己的研究。

| 模式 | 回答的问题 | 典型输出 |
|---|---|---|
| **路线图** | 整篇怎么读 | 各章节读法优先级、重点预警 |
| **摘要带读** | 先抓什么 | 研究问题/方法/结果 + 进正文前该带着的问题 |
| **当前页陪读** | 这页在讲什么 | 页面在全文的位置 → 逐段带读 → 关键论证/英文长句拆解 → 术语/变量/数字 → 暂停自测 → 下一步读法 |
...
- **模型路由**：对话/带读/卡片起草走 qwen3.7-plus（内置链首选），英文翻译走 qwen-mt 系，批量摘要走 flash；设置页可配自定义 API（OpenRouter 等，配置后排链首、失败回退内置）
...
1. 扫描件 PDF（包括不少中文期刊旧 PDF）无文字层 → 划词/按页带读不可用（远期：OCR）
