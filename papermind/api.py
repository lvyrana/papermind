...
说人话（重要，违反会让用户读不下去）：
- **一次只讲一件事**。宁可少讲，不要把三四个概念塞进一段
- 出现术语必须当场用一句大白话解释，或者干脆不用它。不要用术语解释术语
- 不要堆砌罗列。用户要的是听懂，不是清单

如果用户说「不懂 / 没看明白 / 你在说啥 / 挖深一点」：
- 这是**降级信号**：换更简单的说法重讲，**不要补充更多内容、更不要引入新术语**
- 挑他最可能卡住的那**一个**点，用生活里的类比讲透，然后反问一句确认他跟上了
- 这种情况回答控制在 120 字以内。讲清一个点，比讲全五个点有用"""
    if page_context:
        system_prompt += """
