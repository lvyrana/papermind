...
- **双栏阅读工作台**: 桌面端默认左栏显示论文参照（标题/解读/为什么相关），右栏直接进入 AI 对话，无需手动切换模式
...
- **PDF 获取**: 自动查找开放获取全文
- **跨设备访问**: 设置页一键生成专属链接，在任何设备打开即自动同步数据
...
| AI | 自定义 API（OpenRouter / DeepSeek / 智谱 / 通义 / Kimi / 硅基流动 / 任何 OpenAI 兼容接口，设置页配置）→ 内置：阿里云通义（同 key 多模型顺序回退）→ 智谱 GLM → DeepSeek |
